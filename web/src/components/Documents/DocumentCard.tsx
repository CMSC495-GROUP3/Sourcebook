    ...
    <li>
      <button
        ...
        className={`flex w-full cursor-pointer items-start gap-3 rounded-md px-3 py-3 text-left transition-colors ${
          selected ? 'bg-accent-soft' : 'hover:bg-ink/5'
        }`}
      >
        <FileText
          size={16}
          aria-hidden="true"
          className={`mt-0.5 shrink-0 ${selected ? 'text-accent' : 'text-ink-3'}`}
        />
        <span className="flex min-w-0 flex-1 flex-col gap-1">
          ...
          <span className="tnum flex flex-wrap items-center gap-x-2 text-[11.5px] text-ink-3">
            {document.category && <span className="caps text-[10px] text-accent">{document.category}</span>}
            <span>{meta}</span>
          </span>
        </span>
      </button>
