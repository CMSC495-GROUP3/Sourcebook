...
| Reload the page | the conversation and its sources are restored from history | Pass. Same URL, both turns and their chips back. `06` |
...
| Refusal card after a coverage-judge refusal, found in this pass, no issue yet | a question refused by the judge shows its cosine score as "Strong match · 79%" under "No matching policy", and the card says nothing indexed came close. Both are true only of a cosine-gate refusal | the refusal itself and the Ask Human Resources button are correct; only the badge and one sentence mislead |
